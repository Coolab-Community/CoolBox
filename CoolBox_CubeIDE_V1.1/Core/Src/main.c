...
int main(void)
{
  ...
  /* USER CODE BEGIN RTOS_THREADS */
  	...
  	/* MODBUS TCP/IP slave : register adc values in HOLDING REGISTER 0 and 1 */
    osThreadDef(Modbus_Task, ModbusTask, osPriorityNormal, 0, 1024);
    Task2 = osThreadCreate(osThread(Modbus_Task), NULL);

    /*ADC Management : Send mailQ to the Model.cpp with Adc values*/
    osThreadDef(Task2, ADC_Task, osPriorityNormal, 0, 1024);
    osThreadCreate(osThread(Task2), NULL);

    /* IDWG Management : Refresh the IWDG : if IWDG timer reach 10s => Reset */
    osThreadDef(IWDG, IWDG_Task, osPriorityNormal, 0, 128);
    osThreadCreate(osThread(IWDG), NULL);
